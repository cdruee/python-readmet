...
function version () {
  # author/email/description are persistent metadata and now live in
  # _metadata.py; _version.py itself is fully regenerated by
  # setuptools_scm on every build and contains only __version__.
  grep $1 ${NAME}/_metadata.py | sed 's/.*'\''\(.*\)'\''.*/\1/'
}
